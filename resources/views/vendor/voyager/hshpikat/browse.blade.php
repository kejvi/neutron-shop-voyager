...
@section('page_header')
    <div class="container-fluid">
        ...
        @endcan
        {{--@can('add', app($dataType->model_name))--}}
            <a id="bulkTransferBtn" class="btn btn-primary ">
                <i class="voyager-plus"></i> <span>{{'Transfero' }}</span>
            </a>
        {{--@endcan--}}
...
@section('content')
    ...
    {{--<a class="btn btn-danger" id="bulkTransferBtn"><i class="voyager-trash"></i> <span>{{ __('voyager::generic.bulk_delete') }}</span></a>--}}
    {{-- zgjidh post office modal --}}
    <div class="modal modal-primary fade" tabindex="-1" id="bulk_transfer_modal" role="dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="voyager-info-circled"></i> {{ 'Jeni i sigurt qe doni te beni transferimin' }} <span id="bulk_delete_count"></span> <span id="bulk_delete_display_name"></span>?
                    </h4>
                </div>
                ...
                <div class="modal-footer">
                    <form action="{{ route('transfer-to-office') }}" id="bulk_delete_form" method="POST">
                        {{ method_field("POST") }}
                        {{ csrf_field() }}
                        <input type="hidden" name="ids" id="bulk_transfer_input" value="">
                        <input name="postal_code" type="number" class="number pull-left" id="postal_code" placeholder="Kodi Postar">
                        <input type="submit" class="btn btn-primary pull-right "
                               value="{{'Konfirmo Transfermin' }}">
                    </form>
...
@section('javascript')
    ...
    <script>
        ...
        var $bulkDeleteBtn = $('#bulkTransferBtn');
        var $bulkDeleteModal = $('#bulk_transfer_modal');
        var $bulkDeleteCount = $('#bulk_delete_count');
        var $bulkDeleteDisplayName = $('#bulk_delete_display_name');
        var $bulkDeleteInput = $('#bulk_transfer_input');
        ...
        $bulkDeleteBtn.click(function () {
            ...
            if (count) {
                ...
                // No row selected
                toastr.warning('{{ 'Nuk keni zgjedhur asnje produkt per transferim zyre' }}');
            }
        });
    </script>
@stop
